...
global {
	...
	int agenteEnZonaSegura <- 0;
	int worldSize <- 10;
	float saveDistance <- 4.98; //4.98
	float headingChange <- 70.02; //70.02
	float randomVariation <- 4.5; //4.5
	init {
	...
	//write agentsDeath;
		...
		loop ag over: agentes {
			...
			if (euclideanDistance(myAgentPosition, closestAgentPosition) < saveDistance) {
				...
				agenteEnZonaSegura <- agenteEnZonaSegura + 1;
			}
	...
	output {
		...
		display chart {
			chart "Grafica" type: series {
				...
				//data "Tiempo de agente en zona segura: " value: agenteEnZonaSegura color: #green;
			}
